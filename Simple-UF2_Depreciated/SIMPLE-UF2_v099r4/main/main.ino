...
void run_once() {
  ...
  if (!EEPROM_INITIAL_WRITE) {
    ...
    if (usb_voltage_check == HIGH) {
      ...
      USB_STRAP_TEST = 1;  //strap is fitted. Lets store to flash
      EEPROM_INITIAL_WRITE = 1; // run-once complete. Store to flash to say it has ran
    } else {
      delayMicroseconds(2000000);
      USB_STRAP_TEST = 0; //strap is not fitted. Lets store to flash
      EEPROM_INITIAL_WRITE = 1; // run-once complete. Store to flash to say it has ran
    }
...
if (!JOYCON_STRAP_TEST){
  attachInterrupt(JOYCON_STRAP_PIN, isfitted, LOW);
}
#endif
#ifdef MODE_CHANGE_PIN
